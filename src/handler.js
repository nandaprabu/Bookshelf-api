...
const allBookHandler = (request, h) => {
  ...
  if (name !== undefined) {
    ...
    const bookStatus = books.filter((book) => Number(book.finished) === Number(finished));
    const response = h.response({
      status: 'success',
      data: {
        books: bookStatus.map((book) => ({
          id: book.id,
          name: book.name,
          publisher: book.publisher,
        })),
...
module.exports = { addBookHandler, allBookHandler, getBookIdHandler, editBookHandler, deleteBookHandler };
